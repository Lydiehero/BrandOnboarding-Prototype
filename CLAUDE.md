...
**Canonical patterns:**
- Header back navigation → `<AkButton outlined symbol="chevron-left">Back</AkButton>` (see `OrderDetailPage.vue`)
- Filter reset → `<AkButton outlined>Clear</AkButton>` (see `OrderFilterBar.vue`)
- Form actions (standard pair) → `[Cancel · outlined] [Save · primary]` side by side
- Wizard step footer → `[← Back · outlined] [Continue · primary]`
- Final step with save-for-later → `[← Back · outlined] [Save & finish later · outlined] [Submit · primary]`
